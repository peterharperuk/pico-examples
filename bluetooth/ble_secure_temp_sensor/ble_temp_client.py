...
Security settings (must match the SECURITY_SETTING the Pico was built with):
    0 - Just Works        (no MITM, no I/O required)
    1 - Numeric Comparison (confirm passkey displayed by server)
    2 - Passkey Entry      (type passkey displayed by server)
    3 - Passkey Display    (server types passkey displayed here)

Usage:
    python ble_temp_client.py [--security <0-3>] [--scan-timeout <seconds>] [--debug]
...
SECURITY_HINTS = {
    ...
    1: ("Numeric Comparison - the Pico will display a number on its serial "
        "console.  Your OS will ask you to confirm it matches."),
    2: ("Passkey Entry - the Pico will display a 6-digit passkey on its "
        "serial console.  Enter it when prompted by your OS."),
    3: ("Passkey Display - your host will display a passkey; "
        "enter it on the Pico's serial console."),
}
...
async def run(security: int, timeout: float) -> None:
    log.info("Security setting %d: %s", security, SECURITY_HINTS.get(security, "?"))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Security setting (must match the Pico build).  Default: 0 (Just Works).",
    )
    parser.add_argument(
    ...
    try:
        asyncio.run(run(security=args.security, timeout=args.scan_timeout))
